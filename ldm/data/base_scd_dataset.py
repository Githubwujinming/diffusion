...
class SCDBaseDataSet(Dataset):
    ...
    def _crop(self, image_A, image_B, A_label=None, B_label=None):   
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            A_label   = cv2.copyMakeBorder(A_label, value=0, **pad_kwargs) #use 0 for padding
            B_label   = cv2.copyMakeBorder(B_label, value=0, **pad_kwargs) #use 0 for padding
        # Cropping 
        h, w, _ = image_A.shape
        ...
        A_label = A_label[start_h:end_h, start_w:end_w]
        B_label = B_label[start_h:end_h, start_w:end_w]
        return image_A, image_B, A_label, B_label
    ...
    def _flip(self, image_A, image_B, A_label=None, B_label=None):
        # Random H flip
        if random.random() > 0.5:
            image_A = np.fliplr(image_A).copy()
            image_B = np.fliplr(image_B).copy()
            A_label = np.fliplr(A_label).copy()
            B_label = np.fliplr(B_label).copy()
        return image_A, image_B, A_label,B_label

    def _resize(self, image_A, image_B, A_label=None, B_label=None, bigger_side_to_base_size=True):
        if isinstance(self.base_size, int):
            ...
            A_label = cv2.resize(A_label, (w, h), interpolation=cv2.INTER_NEAREST)
            B_label = cv2.resize(B_label, (w, h), interpolation=cv2.INTER_NEAREST)
            return image_A, image_B, A_label, B_label
    ...
    def _val_augmentation(self, image_A, image_B, A_label, B_label):
        if self.base_size is not None:
            image_A, image_B, A_label, B_label = self._resize(image_A, image_B, A_label, B_label)
    ...
    def _augmentation(self, image_A, image_B, A_label, B_label):
        h, w, _ = image_A.shape
        ...
        if self.crop_size is not None:
            image_A, image_B, A_label, B_label = self._crop(image_A, image_B, A_label, B_label)

        if self.flip:
            image_A, image_B, A_label, B_label = self._flip(image_A, image_B, A_label, B_label)
        ...
        return self.normalize(self.to_tensor(image_A)), self.normalize(self.to_tensor(image_B)), A_label, B_label
    ...
    def __getitem__(self, index):
        ...
        if self.val:
            ...
            image_A, image_B, A_label, B_label = self._augmentation(image_A, image_B, A_label, B_label)
